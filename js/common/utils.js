...
define(function (uf, require) {
    ...
    // 获取时间差
    let getTimeDiff = function (start, end) {
        let t1 = uf.moment(start);
        let t2 = uf.moment(end);
        if (!t2) {
            return '填写结束时间哦~';
        }
        let diffHours = uf.moment(new Date(t2) - new Date(t1)) / 1000 / 60 / 60;
    ...
    return {
        creatUpdateStore, addDataStore, getStoreData, delectStoreData, updateStoreData,
        storeDataList, getTimeDiff, batchDelete
    };
});